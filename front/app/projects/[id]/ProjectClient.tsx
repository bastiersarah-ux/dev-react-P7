...
import { Project, ProjectMember, Task, TaskStatus } from '@front/types/api-types';
				...
				<div className='navbar flex-wrap gap-2 bg-gray-100  rounded-box px-4 py-5'>
					...
					<div className='flex items-center gap-2'>
						...
						{members
							...
							.map((member) => (
								<div key={member.id} className='flex items-center gap-2 h-6.75'>
									<UserInitialsButton user={member.user} showFull variant={'Variant2'} />
								</div>
							))}
					</div>
				</div>
				...
				<div className='card bg-white border border-gray-200 p-6 gap-5'>
					<div className='flex items-center gap-4 flex-wrap justify-between w-full'>
						...
						<div className='flex items-center gap-4 flex-wrap'>
							...
							<select value={undefined} className='select select-lg h-11.25 w-31 pr-5'>
					...
					{(!tasks || tasks.length === 0) && <p className='text-sm text-gray-500 text-center p-15'>Aucune tâche assignée.</p>}
								...
								<div className='collapse-content mt-6'>
									...
									<div className='flex gap-3 mt-6 items-center'>
										<div className='flex w-16 h-14'>
											<UserInitialsButton key={project?.owner?.id} user={project?.owner} fullWidth />
										</div>
										<input type='text' className='input bg-gray-50 h-15 w-full' placeholder='Ajouter un commentaire...' />
										<button className='btn btn-primary btn-outline'>Envoyer</button>
									</div>
								</div>
							</details>
						</div>
